$(".btn").mouseup(function() {
  this.blur();
}); //removes focus on buttons after clicked.

$(document).ready(function() {
  // function run at after page load
  ...
  function quoteFinder() {
    $.ajax({
      url:
        "https://api.forismatic.com/api/1.0/?method=getQuote&format=jsonp&lang=en", // info at end of url can also be adjusted in the data object a few lines down.
      jsonp: "jsonp",
      dataType: "jsonp",
      data: {
        method: "getQuote",
    ...
    if (authorPull) {
      window.open(
        "https://twitter.com/intent/tweet?text=" +
          encodeURIComponent('"' + quotePull + '"' + " - " + authorPull)
      );
    } else {
      window.open(
        "https://twitter.com/intent/tweet?text=" +
          encodeURIComponent('"' + quotePull + '"' + " - unknown")
      );
    }
  });
